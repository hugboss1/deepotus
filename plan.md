...
(Sprints 6 → 13.3 + Infiltration 14.x + Brain Connect 15.x + Déploiements 17 + Hardening 22.x)
...
- **Nouvel objectif (Hardening 22.x → 24)** :
  - Poursuivre le durcissement TS (vers `noImplicitAny: true`, puis `strictNullChecks`), **sans bascule brutale**.
  - Augmenter la valeur “robustness” via tests : **pytest** backend + **Playwright E2E**.
...
- ✅ Intégration templates (loyalty/welcome/access card/genesis broadcast).
...
- ✅ Source of truth phases : `frontend/src/lib/launchPhase.ts`.
- ✅ Backend treasury ops : `GET /api/treasury/operations` + burns + admin CRUD.
- ✅ Page `/transparency` + placeholders pre-mint ; enrichissements post-mint.
- ✅ Hero 3 phases + countdown.
- ✅ Tokenomics locks + `/transparency`.
- ✅ HowToBuy phase-aware.
- ✅ Roadmap phase-aware.
- ✅ BurnCounter (`/api/treasury/burns`).
- ✅ Docs mises à jour.

#### Sprint 21 — Refactor (zero new feature) — ✅ COMPLET
- ✅ `AdminBots.jsx` 1798 → 1017 lignes via extraction sections.
- ✅ `cadence_engine.py` split + **25 tests** `test_cadence_engine_helpers.py`.

#### Sprint 22 — Migration TypeScript (soft) — ✅ 22.1 + ✅ 22.2 + ✅ 22.3
- ✅ 22.1 : `index.tsx`, `App.tsx`, `AdminBots.tsx`.
- ✅ 22.2 : `AdminVault.tsx` (constaté déjà terminé ; `tsc --noEmit` clean).
- ✅ 22.3 (nouveau) : extraction Custom LLM keys
  - Nouveau : `src/pages/admin/sections/CustomLlmKeysSection.tsx`
  - `AdminBots.tsx` réduit **1037 → 743** (-28%).
  - Tous `data-testid` préservés ; test visuel validé.

#### Sprint 22.4 — Hardening TS (allégé) — ✅ LIVRÉ
- Correction ciblée du top 10 fichiers à `noImplicitAny` :
  - CombinationDial, AdminBots, Propaganda, ROISimulator, PriceChart, PublicStats, AuthedVaultView, CabinetVault, AdminVault, Infiltration.
- Score `npx tsc --noEmit --noImplicitAny` : **274 → 140** erreurs (-49%).
- `tsconfig.json` **inchangé** (strict reste `false`) : bascule reportée à Sprint 23.

#### Sprint 22.5 — Tests (pytest + Playwright bootstrap) — ✅ LIVRÉ
- Pytest backend : **25 → 57** tests (ajout : `whale_watcher` + `clearance_levels` helpers).
- Playwright E2E bootstrap : `/app/e2e/`
  - 3 smoke tests (landing + ROI + admin Custom LLM keys)
  - `npx playwright test --list` OK.
- Doc : `/app/docs/SPRINT_22_3_5_DEPLOY.md`.
...
### Phase 22 — TypeScript Hardening & Tests (nouveau bloc)

#### Sprint 22.3 (P1) — Extraction Custom LLM keys ✅ **COMPLETED**
- Extraire la section + dialog "Custom LLM keys" dans `CustomLlmKeysSection.tsx`.
- Maintenir les `data-testid` et valider via smoke test visuel.

#### Sprint 22.4 (P3) — Hardening TS progressif ✅ **COMPLETED (allégé)**
- Réduire les erreurs `noImplicitAny` sur les fichiers à plus fort ROI.
- Ne pas activer `strict` tant que la migration legacy n’est pas 100% stabilisée.

#### Sprint 22.5 (P4) — Tests automatisés ✅ **COMPLETED (bootstrap)**
- Pytest : ajouter tests unitaires pour helpers purs (whale_watcher + clearance_levels).
- Playwright : bootstrap E2E minimal (landing + ROI + admin bots).
...
### P1/P2 — Stabilisation & qualité
- **Sprint 23 (P3)** — terminer les **140 erreurs** restantes sous `--noImplicitAny`, puis basculer :
  - `noImplicitAny: true` (d’abord)
  - puis `strictNullChecks: true` (séparément)
  - puis `strict: true` (optionnel, dernière étape)
- **Sprint 24 (P4)** — intégrer Playwright en CI (GitHub Actions) :
  - variable `PLAYWRIGHT_BASE_URL`
  - stratégie (preview vs localhost)
  - artefacts (report HTML) + traces on failure.

### P2 — Refactors prudents (après Sprint 23 idéalement)
...
- Garder “behaviour parity” et verrouiller via tests Playwright.
...
- **Qualité & hardening** :
  ...
  - `pytest` backend 
    - ✅ 57 tests passent (baseline actuelle)
    - objectif Sprint 23/24 : étendre vers endpoints critiques (vault, propaganda, treasury).
  - Playwright E2E
    - ✅ bootstrap présent dans `/app/e2e/`
    - objectif Sprint 24 : exécution CI + couverture minimale (transparency + admin vault + propaganda queue).

---

## 5) Notes d’architecture (Phase 13–24)

**Backend**
- ✅ Propaganda : orchestrateur + triggers + queue + templates + tone engine.
- ✅ 13.3 : dispatchers + worker APScheduler + routes admin + doc ops.
- ✅ 13.3.x : retry/backoff + preflight creds + diagnostics état.
...
- ✅ Tests :
  ...
  - `backend/tests/test_whale_watcher_helpers.py` (nouveau)
  - `backend/tests/test_clearance_levels_helpers.py` (nouveau)

**Frontend**
- ✅ Pages admin : `pages/Propaganda.tsx`, `pages/Infiltration.tsx`, `pages/CabinetVault.tsx`, `pages/AdminVault.tsx`.
- ✅ Propaganda UI : bannière d’état dispatch.
...
- ✅ Hardening ciblé : top 10 fichiers nettoyés pour `--noImplicitAny`.

**E2E (Playwright)**
- ✅ Bootstrap : `/app/e2e/`
- 3 smoke tests : landing + ROI + admin Custom LLM keys.
- Sprint 24 : wiring CI + extension coverage.
